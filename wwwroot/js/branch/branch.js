...
align-items:center;'>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                if (!foundNearby) {
                    $('#branch-list').empty();
                    $('#branch-list').html(`
                        <div style="grid-column: 1 / -1; text-align: center; padding: 40px; background: white; border-radius: 15px; box-shadow: 0 5px 20px rgba(0, 0, 0, 0.08);">
   ...
   $.ajax({
            url: "http://localhost:8085/api/states",
            method: "GET",
            success: function(response) {
   ...
   // When a state is selected, fetch branches for that state
                $('#states').on('change', function() {
                    ...
                    $.ajax({
                        url: "http://localhost:8085/api/selected?state_id=" + stateId,
                        method: "GET",
                        success: function(response) {
                            let branches = [];
                            if (Array.isArray(response)) {
